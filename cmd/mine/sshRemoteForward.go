...
func (w *worker) sshRemoteForward(ctx context.Context, sshKeyFile string) error {
	...
	args := []string{"-v", "-i", sshKeyFile, "-N", "-R", fmt.Sprintf("/home/%s/notebook.sock:127.0.0.1:8889", w.jID), "-p", "2222", fmt.Sprintf("%s@notebook.emrys.io", w.jID), "-o", "StrictHostKeyChecking=no", "-o", "UserKnownHostsFile=/dev/null"}
